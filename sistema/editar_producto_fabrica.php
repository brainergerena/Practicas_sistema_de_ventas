...
if (!empty($_POST)) {
  ...
  if (empty($_POST['producto']) || empty($_POST['precio'])) {
    ...
    $query_update = mysqli_query($conexion, "UPDATE fabrica SET descripcion = '$producto', proveedor= $proveedor,precio= $precio WHERE codfabrica = $codfabrica");
    if ($query_update) {
      $alert = '<div class="alert alert-primary" role="alert">
              Modificado
            </div>';
...
if (empty($_REQUEST['id'])) {
  ...
  $query_producto = mysqli_query($conexion, "SELECT p.codfabrica, p.descripcion, p.precio, pr.codproveedor, pr.proveedor FROM fabrica p INNER JOIN proveedor pr ON p.proveedor = pr.codproveedor WHERE p.codfabrica = $id_producto");
